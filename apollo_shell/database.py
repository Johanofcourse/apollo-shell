...
class OutageDatabase:
    ...
    def create_tables(self):
        ...
        # Choctawhatchee Electric Cooperative (CHELCO) raw snapshots -
        # same vendor platform/shape as TCEC/EREC, a combined-territory
        # source, always exactly one row (see
        # fetch_chelco_outages.COMBINED_TERRITORY_LABEL - real counties:
        # Santa Rosa, Okaloosa, Walton, Holmes), kept in its own table
        # per the same one-utility-per-table convention.
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS chelco_outages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp TEXT NOT NULL,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
        ...
        # CHELCO lifecycle tracking - identical algorithm to
        # erec_outage_events/tcec_outage_events (open on
        # customers_out > 0, track peak, close on return to 0).
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS chelco_outage_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
                start_time TEXT NOT NULL,
        ...
        # Gulf Coast Electric Cooperative (GCEC) raw snapshots - same
        # vendor platform/shape as TCEC/EREC/CHELCO, a combined-territory
        # source, always exactly one row (see
        # fetch_gcec_outages.COMBINED_TERRITORY_LABEL - real counties:
        # Bay, Calhoun, Gulf, Jackson, Walton, Washington), kept in its
        # own table per the same one-utility-per-table convention.
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS gcec_outages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp TEXT NOT NULL,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
        ...
        # GCEC lifecycle tracking - identical algorithm to
        # chelco_outage_events/erec_outage_events (open on
        # customers_out > 0, track peak, close on return to 0).
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS gcec_outage_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
                start_time TEXT NOT NULL,
        ...
        # Orlando Utilities Commission (OUC) raw snapshots - a real
        # single-county rollup like FKEC/PRECO/LWBU, always exactly one
        # row (see fetch_ouc_outages.SERVICE_COUNTY: Orange), kept in
        # its own table per the same one-utility-per-table convention.
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS ouc_outages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp TEXT NOT NULL,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
        ...
        # OUC county-level lifecycle tracking - identical algorithm to
        # fkec_outage_events/lwbu_outage_events (open on customers_out >
        # 0, track peak, close on return to 0).
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS ouc_outage_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
                start_time TEXT NOT NULL,
        ...
        # Lee County Electric Cooperative (LCEC) raw snapshots - a real
        # per-county rollup like Talquin/PRECO, but sourced from a
        # fully public S3/CloudFront-hosted JSON file rather than a
        # WAF-gated API (see fetch_lcec_outages.py).
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS lcec_outages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp TEXT NOT NULL,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
        ...
        # LCEC county-level lifecycle tracking - identical algorithm to
        # ouc_outage_events/fkec_outage_events (open on customers_out >
        # 0, track peak, close on return to 0).
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS lcec_outage_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                utility TEXT NOT NULL,
                county TEXT NOT NULL,
                start_time TEXT NOT NULL,
        ...
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_chelco_outage_events_open
            ON chelco_outage_events(utility, county, end_time)
        ''')
        ...
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_ouc_outage_events_open
            ON ouc_outage_events(utility, county, end_time)
        ''')
        ...
        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_chelco_outages_unique
            ON chelco_outages(timestamp, utility, county)
        ''')

        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_chelco_outage_events_unique
            ON chelco_outage_events(utility, county, start_time)
        ''')

        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_gcec_outages_unique
            ON gcec_outages(timestamp, utility, county)
        ''')
        ...
        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_ouc_outages_unique
            ON ouc_outages(timestamp, utility, county)
        ''')

        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_ouc_outage_events_unique
            ON ouc_outage_events(utility, county, start_time)
        ''')

        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_lcec_outages_unique
            ON lcec_outages(timestamp, utility, county)
        ''')
    ...
    def log_chelco_outages(self, outage_list, timestamp=None):
        """
        Insert Choctawhatchee Electric Cooperative's raw combined-
        territory snapshot (from fetch_chelco_outages.py) - always
        exactly one record (see
        fetch_chelco_outages.COMBINED_TERRITORY_LABEL). Same
        shape/principle as log_erec_outages(), kept in its own table.
        ...
        for outage in outage_list:
            ...
            customers_served = outage['customers_served']
            percentage_out = (customers_out / customers_served * 100) if customers_served > 0 else 0
            records.append((timestamp, "Choctawhatchee Electric Cooperative, Inc.", outage['county'], customers_out, customers_served, percentage_out))

        cursor.executemany('''
            INSERT OR IGNORE INTO chelco_outages (timestamp, utility, county, customers_out, customers_served, percentage_out)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', records)

        conn.commit()
        print(f"Logged {len(records)} CHELCO outage records")


    def sync_chelco_outage_events(self, outage_list, timestamp=None):
        """
        Update chelco_outage_events from a fresh batch of snapshots.
        Identical algorithm to sync_erec_outage_events() - kept as
        CHELCO's own dedicated table/method, same one-utility-per-table
        convention.
        ...
        timestamp = timestamp or datetime.now().isoformat()
        utility = "Choctawhatchee Electric Cooperative, Inc."
        ...
        for outage in outage_list:
            ...
            cursor.execute('''
                SELECT id, peak_customers_out FROM chelco_outage_events
                WHERE utility = ? AND county = ? AND end_time IS NULL
            ''', (utility, county))
            open_event = cursor.fetchone()
            ...
            if customers_out > 0:
                if open_event is None:
                    cursor.execute('''
                        INSERT OR IGNORE INTO chelco_outage_events
                            (utility, county, start_time, end_time, peak_customers_out, peak_percentage_out, customers_served)
                        VALUES (?, ?, ?, NULL, ?, ?, ?)
                    ''', (utility, county, timestamp, customers_out, percentage_out, customers_served))
                    if cursor.rowcount > 0:
                ...
                if open_event is not None:
                    cursor.execute('''
                        UPDATE chelco_outage_events SET end_time = ? WHERE id = ?
                    ''', (timestamp, open_event['id']))
                    closed += 1

        conn.commit()
        print(f"CHELCO outage events: {opened} opened, {closed} closed this cycle")


    def log_gcec_outages(self, outage_list, timestamp=None):
        """
        Insert Gulf Coast Electric Cooperative's raw combined-
        territory snapshot (from fetch_gcec_outages.py) - always
        exactly one record (see
        fetch_gcec_outages.COMBINED_TERRITORY_LABEL). Same
        shape/principle as log_chelco_outages(), kept in its own table.
        ...
        for outage in outage_list:
            ...
            customers_served = outage['customers_served']
            percentage_out = (customers_out / customers_served * 100) if customers_served > 0 else 0
            records.append((timestamp, "Gulf Coast Electric Cooperative, Inc.", outage['county'], customers_out, customers_served, percentage_out))

        cursor.executemany('''
            INSERT OR IGNORE INTO gcec_outages (timestamp, utility, county, customers_out, customers_served, percentage_out)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', records)

        conn.commit()
        print(f"Logged {len(records)} GCEC outage records")


    def sync_gcec_outage_events(self, outage_list, timestamp=None):
        """
        Update gcec_outage_events from a fresh batch of snapshots.
        Identical algorithm to sync_chelco_outage_events() - kept as
        GCEC's own dedicated table/method, same one-utility-per-table
        convention.
        ...
        timestamp = timestamp or datetime.now().isoformat()
        utility = "Gulf Coast Electric Cooperative, Inc."
        ...
        for outage in outage_list:
            ...
            cursor.execute('''
                SELECT id, peak_customers_out FROM gcec_outage_events
                WHERE utility = ? AND county = ? AND end_time IS NULL
            ''', (utility, county))
            open_event = cursor.fetchone()
            ...
            if customers_out > 0:
                if open_event is None:
                    cursor.execute('''
                        INSERT OR IGNORE INTO gcec_outage_events
                            (utility, county, start_time, end_time, peak_customers_out, peak_percentage_out, customers_served)
                        VALUES (?, ?, ?, NULL, ?, ?, ?)
                    ''', (utility, county, timestamp, customers_out, percentage_out, customers_served))
                    if cursor.rowcount > 0:
                ...
                if open_event is not None:
                    cursor.execute('''
                        UPDATE gcec_outage_events SET end_time = ? WHERE id = ?
                    ''', (timestamp, open_event['id']))
                    closed += 1
    ...
    def log_ouc_outages(self, outage_list, timestamp=None):
        """
        Insert Orlando Utilities Commission's raw single-county snapshot
        (from fetch_ouc_outages.py) - always exactly one record (see
        fetch_ouc_outages.SERVICE_COUNTY). Same shape/principle as
        log_fkec_outages()/log_lwbu_outages(), kept in its own table.
        ...
        for outage in outage_list:
            ...
            customers_served = outage['customers_served']
            percentage_out = (customers_out / customers_served * 100) if customers_served > 0 else 0
            records.append((timestamp, "Orlando Utilities Commission", outage['county'], customers_out, customers_served, percentage_out))

        cursor.executemany('''
            INSERT OR IGNORE INTO ouc_outages (timestamp, utility, county, customers_out, customers_served, percentage_out)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', records)

        conn.commit()
        print(f"Logged {len(records)} OUC outage records")


    def sync_ouc_outage_events(self, outage_list, timestamp=None):
        """
        Update ouc_outage_events from a fresh batch of snapshots.
        Identical algorithm to sync_lwbu_outage_events() - kept as
        OUC's own dedicated table/method, same one-utility-per-table
        convention.
        ...
        for outage in outage_list:
            ...
            cursor.execute('''
                SELECT id, peak_customers_out FROM ouc_outage_events
                WHERE utility = ? AND county = ? AND end_time IS NULL
            ''', (utility, county))
            open_event = cursor.fetchone()
            ...
            if customers_out > 0:
                if open_event is None:
                    cursor.execute('''
                        INSERT OR IGNORE INTO ouc_outage_events
                            (utility, county, start_time, end_time, peak_customers_out, peak_percentage_out, customers_served)
                        VALUES (?, ?, ?, NULL, ?, ?, ?)
                    ''', (utility, county, timestamp, customers_out, percentage_out, customers_served))
                    if cursor.rowcount > 0:
                ...
                if open_event is not None:
                    cursor.execute('''
                        UPDATE ouc_outage_events SET end_time = ? WHERE id = ?
                    ''', (timestamp, open_event['id']))
                    closed += 1

        conn.commit()
        print(f"OUC outage events: {opened} opened, {closed} closed this cycle")


    def log_lcec_outages(self, outage_list, timestamp=None):
        """
        Insert Lee County Electric Cooperative's raw per-county
        snapshot (from fetch_lcec_outages.py). Same shape/principle as
        log_ouc_outages()/log_fkec_outages(), kept in its own table.
        ...
        for outage in outage_list:
            ...
            customers_served = outage['customers_served']
            percentage_out = (customers_out / customers_served * 100) if customers_served > 0 else 0
            records.append((timestamp, "Lee County Electric Cooperative", outage['county'], customers_out, customers_served, percentage_out))

        cursor.executemany('''
            INSERT OR IGNORE INTO lcec_outages (timestamp, utility, county, customers_out, customers_served, percentage_out)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', records)

        conn.commit()
        print(f"Logged {len(records)} LCEC outage records")


    def sync_lcec_outage_events(self, outage_list, timestamp=None):
        """
        Update lcec_outage_events from a fresh batch of snapshots.
        Identical algorithm to sync_ouc_outage_events() - kept as
        LCEC's own dedicated table/method, same one-utility-per-table
        convention.
        ...
        for outage in outage_list:
            ...
            cursor.execute('''
                SELECT id, peak_customers_out FROM lcec_outage_events
                WHERE utility = ? AND county = ? AND end_time IS NULL
            ''', (utility, county))
            open_event = cursor.fetchone()
            ...
            if customers_out > 0:
                if open_event is None:
                    cursor.execute('''
                        INSERT OR IGNORE INTO lcec_outage_events
                            (utility, county, start_time, end_time, peak_customers_out, peak_percentage_out, customers_served)
                        VALUES (?, ?, ?, NULL, ?, ?, ?)
                    ''', (utility, county, timestamp, customers_out, percentage_out, customers_served))
                    if cursor.rowcount > 0:
                ...
                if open_event is not None:
                    cursor.execute('''
                        UPDATE lcec_outage_events SET end_time = ? WHERE id = ?
                    ''', (timestamp, open_event['id']))
                    closed += 1
    ...
    def get_chelco_open_events(self):
        """
        Return currently open chelco_outage_events (end_time IS NULL),
        worst (by peak percentage out) first. Includes
        current_customers_out/current_percentage_out from the latest
        chelco_outages snapshot for that county, alongside the lifecycle
        peak - same "peak vs. right now" reasoning as get_erec_open_events().
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT oe.*,
                   cur.customers_out AS current_customers_out,
                   cur.percentage_out AS current_percentage_out
            FROM chelco_outage_events oe
            LEFT JOIN chelco_outages cur
                ON cur.utility = oe.utility AND cur.county = oe.county
                AND cur.timestamp = (
                    SELECT MAX(timestamp) FROM chelco_outages o2
                    WHERE o2.utility = oe.utility AND o2.county = oe.county
                )
            WHERE oe.end_time IS NULL
            ORDER BY oe.peak_percentage_out DESC
    ...
    def get_chelco_recent_closed_events(self, limit=10):
        """
        Return the most recently closed chelco_outage_events.
        """
        conn = self.connect()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT * FROM chelco_outage_events
            WHERE end_time IS NOT NULL
            ORDER BY end_time DESC
            LIMIT ?
        ''', (limit,))
    ...
    def get_gcec_open_events(self):
        """
        Return currently open gcec_outage_events (end_time IS NULL),
        worst (by peak percentage out) first. Includes
        current_customers_out/current_percentage_out from the latest
        gcec_outages snapshot for that county, alongside the lifecycle
        peak - same "peak vs. right now" reasoning as get_chelco_open_events().
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT oe.*,
                   cur.customers_out AS current_customers_out,
                   cur.percentage_out AS current_percentage_out
            FROM gcec_outage_events oe
            LEFT JOIN gcec_outages cur
                ON cur.utility = oe.utility AND cur.county = oe.county
                AND cur.timestamp = (
                    SELECT MAX(timestamp) FROM gcec_outages o2
                    WHERE o2.utility = oe.utility AND o2.county = oe.county
                )
            WHERE oe.end_time IS NULL
            ORDER BY oe.peak_percentage_out DESC
    ...
    def get_gcec_recent_closed_events(self, limit=10):
        """
        Return the most recently closed gcec_outage_events.
        """
        conn = self.connect()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT * FROM gcec_outage_events
            WHERE end_time IS NOT NULL
            ORDER BY end_time DESC
            LIMIT ?
        ''', (limit,))
    ...
    def get_ouc_open_events(self):
        """
        Return currently open ouc_outage_events (end_time IS NULL),
        worst (by peak percentage out) first. Includes
        current_customers_out/current_percentage_out from the latest
        ouc_outages snapshot for that county, alongside the lifecycle
        peak - same "peak vs. right now" reasoning as get_lwbu_open_events().
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT oe.*,
                   cur.customers_out AS current_customers_out,
                   cur.percentage_out AS current_percentage_out
            FROM ouc_outage_events oe
            LEFT JOIN ouc_outages cur
                ON cur.utility = oe.utility AND cur.county = oe.county
                AND cur.timestamp = (
                    SELECT MAX(timestamp) FROM ouc_outages o2
                    WHERE o2.utility = oe.utility AND o2.county = oe.county
                )
            WHERE oe.end_time IS NULL
            ORDER BY oe.peak_percentage_out DESC
    ...
    def get_ouc_recent_closed_events(self, limit=10):
        """
        Return the most recently closed ouc_outage_events.
        """
        conn = self.connect()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT * FROM ouc_outage_events
            WHERE end_time IS NOT NULL
            ORDER BY end_time DESC
            LIMIT ?
        ''', (limit,))
    ...
    def get_lcec_open_events(self):
        """
        Return currently open lcec_outage_events (end_time IS NULL),
        worst (by peak percentage out) first. Includes
        current_customers_out/current_percentage_out from the latest
        lcec_outages snapshot for that county, alongside the lifecycle
        peak - same "peak vs. right now" reasoning as get_ouc_open_events().
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT oe.*,
                   cur.customers_out AS current_customers_out,
                   cur.percentage_out AS current_percentage_out
            FROM lcec_outage_events oe
            LEFT JOIN lcec_outages cur
                ON cur.utility = oe.utility AND cur.county = oe.county
                AND cur.timestamp = (
                    SELECT MAX(timestamp) FROM lcec_outages o2
                    WHERE o2.utility = oe.utility AND o2.county = oe.county
                )
            WHERE oe.end_time IS NULL
            ORDER BY oe.peak_percentage_out DESC
    ...
    def get_lcec_recent_closed_events(self, limit=10):
        """
        Return the most recently closed lcec_outage_events.
        """
        conn = self.connect()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT * FROM lcec_outage_events
            WHERE end_time IS NOT NULL
            ORDER BY end_time DESC
            LIMIT ?
        ''', (limit,))
    ...
    def get_chelco_outage_detail(self, utility, county, start_time):
        """
        Same idea as get_erec_outage_detail(), for one CHELCO combined-
        territory outage occurrence.
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT * FROM chelco_outage_events WHERE utility = ? AND county = ? AND start_time = ?
        ''', (utility, county, start_time))
        event = cursor.fetchone()
        if event is None:
            return None
        ...
        end_bound = event["end_time"] or datetime.now().isoformat()
        cursor.execute('''
            SELECT timestamp, customers_out, customers_served, percentage_out FROM chelco_outages
            WHERE utility = ? AND county = ? AND timestamp >= ? AND timestamp <= ?
            ORDER BY timestamp
        ''', (utility, county, start_time, end_bound))
        history = [dict(row) for row in cursor.fetchall()]
    ...
    def get_gcec_outage_detail(self, utility, county, start_time):
        """
        Same idea as get_chelco_outage_detail(), for one GCEC combined-
        territory outage occurrence.
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT * FROM gcec_outage_events WHERE utility = ? AND county = ? AND start_time = ?
        ''', (utility, county, start_time))
        event = cursor.fetchone()
        if event is None:
            return None
        ...
        end_bound = event["end_time"] or datetime.now().isoformat()
        cursor.execute('''
            SELECT timestamp, customers_out, customers_served, percentage_out FROM gcec_outages
            WHERE utility = ? AND county = ? AND timestamp >= ? AND timestamp <= ?
            ORDER BY timestamp
        ''', (utility, county, start_time, end_bound))
        history = [dict(row) for row in cursor.fetchall()]
    ...
    def get_ouc_outage_detail(self, utility, county, start_time):
        """
        Same idea as get_lwbu_outage_detail(), for one OUC real-county
        outage occurrence.
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT * FROM ouc_outage_events WHERE utility = ? AND county = ? AND start_time = ?
        ''', (utility, county, start_time))
        event = cursor.fetchone()
        if event is None:
            return None
        ...
        end_bound = event["end_time"] or datetime.now().isoformat()
        cursor.execute('''
            SELECT timestamp, customers_out, customers_served, percentage_out FROM ouc_outages
            WHERE utility = ? AND county = ? AND timestamp >= ? AND timestamp <= ?
            ORDER BY timestamp
        ''', (utility, county, start_time, end_bound))
        history = [dict(row) for row in cursor.fetchall()]
    ...
    def get_lcec_outage_detail(self, utility, county, start_time):
        """
        Same idea as get_ouc_outage_detail(), for one LCEC real-county
        outage occurrence.
        """
        conn = self.connect()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT * FROM lcec_outage_events WHERE utility = ? AND county = ? AND start_time = ?
        ''', (utility, county, start_time))
        event = cursor.fetchone()
        if event is None:
            return None
