...
# code (this repo is public), same as every other utility here. No
# tracking code or auth needed, plain GET with standard
...
# TCEC's live feed reports ONE combined total across its whole real
# territory, with no per-county breakdown in that response. Real
...
# same "wait for a real event" situation FPUC's markers array was in
# before one finally did. A candidate boundary file turned out to be
# the map's background county-line reference layer, not real service
# territory - it includes two Georgia counties, a dead end for real
# coverage.
#
...
def outages_to_records(data):
    """
    Convert TCEC's raw outage-summary JSON into the same list-of-dicts
