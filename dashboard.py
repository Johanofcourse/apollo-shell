...
def _get_cached_correlations(db_path, days):
    ...
    data = (
        ...
        find_clay_correlations(db_path, days=days),
    )
...
def _build_unified_view(open_events, teco_open_events, duke_open_events, jea_open_events, tallahassee_open_events, talquin_open_events, fpuc_open_events, fpuc_open_incidents, preco_open_events, fkec_open_events, tcec_open_events, erec_open_events, chelco_open_events, gcec_open_events, lwbu_open_events, ouc_open_events, lcec_open_events, clay_open_events):
    """
...
def index():
    ...
    clay_correlation = correlation_summary(clay_matches)
    for stats in clay_correlation.values():
        stats["alert_types_display"] = _format_alert_types(stats["alert_types"])
        stats["confidence_display"] = _format_confidence(stats["confidence_breakdown"])
        stats["confidence_bar"] = _confidence_bar_segments(stats["confidence_breakdown"])
    ...
    combined_confidence = _combine_confidence_breakdowns(matches, teco_matches, duke_matches, jea_matches, tallahassee_matches, talquin_matches, fpuc_matches, preco_matches, fkec_matches, tcec_matches, erec_matches, chelco_matches, gcec_matches, lwbu_matches, ouc_matches, lcec_matches, clay_matches)
...
def county_detail():
    ...
    if selected_county:
        ...
        _attach_active_counties(combined_events, selected_county, db)
    ...
    if source in ("teco", "duke", "fpuc_incident", "lwbu_incident"):
        ...
        if incident_id:
            ...
            if raw_detail["events"] or raw_detail["history"]:
                detail = raw_detail
    elif source in ("fpl", "jea", "talquin", "tallahassee", "fpuc", "preco", "fkec", "tcec", "erec", "chelco", "gcec", "lwbu", "ouc", "lcec", "clay"):
        ...
        if county and start_time:
            utility_fns = {
                ...
                "clay": (CLAY_UTILITY_NAME, db.get_clay_outage_detail),
            }
